...
Four package facts that need naming:
...
- **`@angular/platform-browser-dynamic` and `@angular-devkit/build-angular` are gone.** Both were only
  ...
  section 2.4.
- **`angular-cli-ghpages` v3 is the first release that supports Angular 22.** v2.0.3 pinned
  `@angular-devkit/*` to `^18` and `gh-pages@^3`, so it nested a whole second Angular 18 devkit tree.
  v3 widens those ranges to `>=18 <23` and moves to `gh-pages@6.3.0`. After an `npm dedupe` the package
  shares the workspace devkit, and `npm audit` reports 0 vulnerabilities where it reported 3 before.
  Two v3 breaking changes do not touch this repo: the minimum is now Angular 18, and `browserTarget` is
  rejected — the deploy target already used `buildTarget`.
